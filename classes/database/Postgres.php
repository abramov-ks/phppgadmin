...
class Postgres extends ADODB_base {
	...
	function printField($name, $value, $type, $extras = array()) {
		...
		switch (substr($type,0,9)) {
			...
			case 'character':
			case 'character[]':
				$n = substr_count((string)$value, "\n");
				$n = $n < 5 ? 5 : $n;
				$n = $n > 20 ? 20 : $n;
				echo "<textarea name=\"", htmlspecialchars((string)$name), "\" rows=\"{$n}\" cols=\"35\"{$extra_str}>\n";
				echo htmlspecialchars((string)$value);
				echo "</textarea>\n";
				break;
			default:
				echo "<input name=\"", htmlspecialchars((string)$name), "\" value=\"", htmlspecialchars((string)$value), "\" size=\"35\"{$extra_str} />\n";
				break;
		}
	}
